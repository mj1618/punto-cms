...
//        $this->plainPage=true;
    ...
    function postViews($postId){
        ...
        $views[] = (new HeaderItem())->label('Attachments (Images/Documents)');

        foreach($post->attachments()->get() as $att){
            $views[] = (new LinkItem())->label($att->name.' - '.$att->value)->defaultValue('');
        }
        $views[] = (new ButtonItem())->label('New Attachment');
